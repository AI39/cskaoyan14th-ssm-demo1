...
public class ProcessMeasureCheckServiceImpl implements ProcessMeasureCheckService {
    ...
    @Override
    public Vo<ProcessMeasureCheckVo> queryProcessMeasureLeftEmployee(int page, int rows) {
        PageHelper.startPage(page,rows);                                                                                /*这里是设置生成的page数和行数，由前端实现*/

        List<ProcessMeasureCheckVo> processMeasureCheckVoList1 = processMeasureCheckMapper.queryProcessMeasureCheckLeftEmployee();/*调用底层的方法查询到三张表中的符合条件的合并成一个list*/

        PageInfo<ProcessMeasureCheckVo> pageInfo = new PageInfo<>(processMeasureCheckVoList1);                                 /*根据生成到的list再生成一个pageInfo文件*/

        //一个包含数目和当前页对象列表的封装对象，意思是把它封装成page对象
        Vo<ProcessMeasureCheckVo> processMeasureCheckVoList = new Vo<>(pageInfo.getTotal(),pageInfo.getList());
