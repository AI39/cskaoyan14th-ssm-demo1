...
public class FinalCountCheckServiceImpl implements FinalCountCheckService {
    ...
    @Override
    public Vo<FinalCountCheckVo> queryFinalCountLeftEmployee(int page, int rows) {
        PageHelper.startPage(page,rows);                                                                                /*这里是设置生成的page数和行数，由前端实现*/

        List<FinalCountCheckVo> finalCountCheckList1 = finalCountCheckMapper.queryFinalCountCheckLeftEmployee();/*调用底层的方法查询到三张表中的符合条件的合并成一个list*/

        PageInfo<FinalCountCheckVo> pageInfo = new PageInfo<>(finalCountCheckList1);                                 /*根据生成到的list再生成一个pageInfo文件*/

        //一个包含数目和当前页对象列表的封装对象，意思是把它封装成page对象
        Vo<FinalCountCheckVo> finalCountCheckList = new Vo<>(pageInfo.getTotal(),pageInfo.getList());
